...
//        }
    ...
    var sections = [Section.first, Section.second]
    var firstSectionData = Array(0...4).map { Item(value: $0) }
    var secondSectionData = Array(5...10).map { Item(value: $0) }
    var thirdSectionData = [Item]()
...
extension DataViewController {
    @objc func insertItem() {
        firstSectionData.insert(Item(value: 29), at: 2)
        dataSource.apply(dataSnapshot(), animatingDifferences: true)
    }

    @objc func deleteItem() {
        secondSectionData.removeLast()
        dataSource.apply(dataSnapshot(), animatingDifferences: true)
    }

    @objc func moveItem() {
        firstSectionData.shuffle()
        dataSource.apply(dataSnapshot(), animatingDifferences: true)
    }

    @objc func reloadItem() {
        secondSectionData[0].value = Int.random(in: 1...100)
        secondSectionData[1].value = Int.random(in: 1...100)
        dataSource.apply(dataSnapshot(), animatingDifferences: true)
    }

    @objc func insertSection() {
        sections.append(.third)
        thirdSectionData.append(contentsOf: [Item(value: 33),Item(value: 22)])
        dataSource.apply(dataSnapshot(), animatingDifferences: true)
    }
    ...
    @objc func reloadSection() {
        for (index, _) in secondSectionData.enumerated() {
            secondSectionData[index].value = Int.random(in: 1...100)
        }
        dataSource.apply(dataSnapshot(), animatingDifferences: true)
    }
}
